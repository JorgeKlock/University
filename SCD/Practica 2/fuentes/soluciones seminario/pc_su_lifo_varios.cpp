...
//	unique_lock<mutex> guarda( cerrojo_monitor );
   ...
   if ( primera_libre == 0 )//Para varios while pero no en SU pq se pasa primero la señal a los urgentes
      ocupadas.wait( );
...
void ProdCons1SU::escribir( int valor )
{
 ...
 //  unique_lock<mutex> guarda( cerrojo_monitor );
   ...
   if ( primera_libre == num_celdas_total )//Para varios while, pero no en SU!
      libres.wait( );
