...
//
// archivo: pc_sc_lifo_varios.cpp
// Ejemplo de un monitor en C++11 con semántica SC, para el problema
// del productor/consumidor, con varios prod y cons.
// Opcion LIFO (stack)
//
// Historial:
...
// *****************************************************************************
// clase para monitor buffer, version LIFO, semántica SC, varios prod y cons
...
class ProdCons1SC
{
 ...
 int                        // variables permanentes
   ...
   primera_libre ,
   primera_ocupada,
   numero_ocupadas;         //  indice de celda de la próxima inserción
...
int ProdCons1SC::leer(  )
{
   ...
   while ( numero_ocupadas == 0 )//Para varios while
...
void ProdCons1SC::escribir( int valor )
{
   ...
   while ( numero_ocupadas == num_celdas_total )//Para varios while
      libres.wait( guarda );
...
void funcion_hebra_productora( ProdCons1SC * monitor, int identidad )
{
