...
etiq_levantarse = 2,
num_filosofos = 5,
num_procesos = 2 * num_filosofos+1; //añadimos al camarero
...
void funcion_filosofos(int id)
{
	...
	while (true)
	{
		...
		MPI_Ssend(&valor_nulo, 1, MPI_INT, id_camarero, etiq_sentarse, MPI_COMM_WORLD);

		cout << "Filósofo " << id << " solicita ten. izq." << id_ten_izq << endl;
		// ... solicitar tenedor izquierdo (completar)
		int valor_nulo = 0;
		MPI_Ssend(&valor_nulo, 1, MPI_INT, id_ten_izq, 0, MPI_COMM_WORLD);

		cout << "Filósofo " << id << " solicita ten. der." << id_ten_der << endl;
		// ... solicitar tenedor derecho (completar)
		MPI_Ssend(&valor_nulo, 1, MPI_INT, id_ten_der, 0, MPI_COMM_WORLD);
		...
		MPI_Ssend(&valor_nulo, 1, MPI_INT, id_camarero, etiq_levantarse, MPI_COMM_WORLD);
...
{
	int cuenta_sentados = 0;
	int valor;
	MPI_Status estado;
	while (true)
	{
		if(cuenta_sentados < 4)
		...
		//Procesar la informacion segun si se ha pedido sentarse o levantarse
...
int main(int argc, char** argv)
{
	...
	if (num_procesos == num_procesos_actual)
	{
		...
		if (id_propio == id_camarero) //Si es el 10 camarero
			funcion_camarero;
