        ...
        /// Estoy en el nivel 2
        {
            ...
            if(!mapareal) ///NO
            {
                ...
                if(aprendiendo)
                {
                    ...
                    mapaResultado[sensores.mensajeF][sensores.mensajeC]=mapita[fila][columna];
                    this->actual.fila=sensores.mensajeF;
                    this->actual.columna=sensores.mensajeC;
                    ///Copiar el resto del mapa
                     transformacionfila=fila-sensores.mensajeF;
                     transformacioncol= columna-sensores.mensajeC;
                    for(int i=0; i<100; i++)
                    {
                        for(int j=0; j<100; j++)
                        {
...
int precio (unsigned char a){
    int solucion;
    switch (a)
        {
            case 'S': solucion=1; break;
            case 'T': solucion =2; break;
            case 'B': solucion =5; break;
            case 'A': solucion =10; break;
            default: solucion=999;
        }
        return solucion;
}
...
bool ComportamientoJugador::pathFinding_Costo_Uniforme(const estado &origen, const estado &destino, list<Action> &plan) {
  ...
  while (!colap.empty() and (current.st.fila!=destino.fila or current.st.columna != destino.columna)){
		...
		{
		case 0: preciosiguiente=precio(mapaResultado[current.st.fila-1][current.st.columna]); break;
		case 1: preciosiguiente=precio(mapaResultado[current.st.fila][current.st.columna+1]); break;
		case 2: preciosiguiente=precio(mapaResultado[current.st.fila+1][current.st.columna]); break;
		case 3: preciosiguiente=precio(mapaResultado[current.st.fila][current.st.columna-1]); break;
        }
