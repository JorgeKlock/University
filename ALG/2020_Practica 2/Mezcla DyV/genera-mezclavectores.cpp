...
//Compilar con g++ -std=gnu++0x -o mezcla genera-mezclavectores.cpp
//ejecutar con ./mezcla <num_elem> <num_vectores>
...
void merge(int v_inicial, int v_final, int* parcial, int **T, int n)
{
    ...
    if(v_inicial == v_final)
    {
        ...
        cout << "Division izquierda: " << endl;
        for(int i=v_inicial; i<=(v_inicial+v_final)/2; i++)
        {
            for(int j=0; j<n; j++)
            {
                cout << T[i][j] << " ";
        ...
        cout << "Division derecha: " << endl;
        for(int i=((v_inicial+v_final)/2)+1; i<=v_final; i++)
        {
            for(int j=0; j<n; j++)
            {
                cout << T[i][j] << " ";
...
delete [] aux;
    ...
    cout << "Estos son los vectores inicializados: " << endl;
    for (int i=0; i<k; i++) {
     for (int j=0; j<n; j++)
       cout << T[i][j] << " ";
     cout << " " << endl;
