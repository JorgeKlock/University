...
// QAP2.exe 
// QAP2.exe seed
// QAP2.exe seed, tamano_poblacion, p_cruce_agg, p_cruce_age, p_cruce_am, p_mutacion, MAX_EVALUACIONES
...
int main(int argc, char* argv[])
{
	...
	{
		seed = int(argv[0]);
		int poblacion = int(argv[1]);
		float p_cruce_agg = stof(argv[2]);
		float p_cruce_age = stof(argv[3]);
		float p_cruce_am = stof(argv[4]);
		float p_mutacion = stof(argv[5]);
		unsigned int MAX_EVALUACIONES = unsigned int(argv[6]);
	}
	else
	{
		seed = 1;
