...
;;;;;;; (Posicion ?i ?j " "|X|O) representa que la posicion i,j del tablero esta vacia, o tiene una ficha de Clisp
...
(defrule Conectado_es_simetrica
  ...
  (Conectado ?i1 ?j1 ?forma ?i2 ?j2)
    =>
  (assert (Conectado ?i2 ?j2 ?forma ?i1 ?j1))
)

(defrule Crear_EnLinea
  (or
    (Conectado ?i1 ?j1 ?forma ?i2 ?j2)
    (and
        (Conectado ?i1 ?j1 ?forma ?i3 ?j3)
        (Conectado ?i3 ?j3 ?forma ?i2 ?j2)
        (test (or (neq ?i1 ?i2) (neq ?j1 ?j2)))
    )
  )
    =>
...
(defrule redxucir_fichas_sin_colocar
(declare (salience 1))
...
(defrule Crear_2_en_linea
  (declare (salience 2))
  (logical (Posicion ?i1 ?j1 ?ficha))
  (logical (Posicion ?i2 ?j2 ?ficha))
  (test (neq ?ficha " "))
  (EnLinea ?forma ?i1 ?j1 ?i2 ?j2)
    =>
  (assert (2_en_linea ?forma ?i1 ?j1 ?i2 ?j2 ?ficha))
)

(defrule Comprobar_Puede_ganar_poniendo
  (declare (salience 2))
  (logical (2_en_linea ?forma ?i1 ?j1 ?i2 ?j2 ?ficha))
  (logical (EnLinea ?forma ?i1 ?j1 ?i3 ?j3))
  (logical (Posicion ?i3 ?j3 " "))
  (logical (Fichas_sin_colocar ?ficha ?n))
  (test (> ?n 0))
    =>
  (assert (Puede_ganar -1 -1 ?i3 ?j3 ?ficha))
)

(defrule Comprobar_Puede_ganar_moviendo
  (declare (salience 2))
  (logical (2_en_linea ?forma1 ?i1 ?j1 ?i2 ?j2 ?ficha))
  (logical (Conectado ?i1 ?j1 ?forma1 ?i3 ?j3))
  (logical (Posicion ?i3 ?j3 " "))
  (logical (Posicion ?i4 ?j4 ?ficha))
  (Conectado ?i3 ?j3 ?forma2 ?i4 ?j4)
  (test (neq ?forma1 ?forma2))
    =>
  (assert (Puede_ganar ?i4 ?j4 ?i3 ?j3 ?ficha))
)
...
;;;;;;;;;;; CLISP JUEGA CON CRITERIO ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defrule clisp_juega_con_criterio_fichas_sin_colocar_mov_ganador
  (declare (salience -9996))
  ?f <- (Turno X)
  (Fichas_sin_colocar X ?n)
  (Puede_ganar -1 -1 ?i ?j X)
  ?g <- (Posicion ?i ?j " ")
    =>
  (printout t "Juego poner ficha en " ?i ?j crlf)
  (retract ?f ?g)
...
(defrule clisp_juega_con_criterio_fichas_sin_colocar_evitar_mov_ganador
  (declare (salience -9997))
  ?f <- (Turno X)
  (Fichas_sin_colocar X ?n)
  (or
    (Puede_ganar -1 -1 ?i ?j O)
    (Puede_ganar ? ? ?i ?j O)
  )
  ?g <- (Posicion ?i ?j " ")
    =>
...
(defrule clisp_juega_con_criterio_mov_ganador
  (declare (salience -9996))
  ?f <- (Turno X)
  (Todas_fichas_en_tablero X)
...
(defrule clisp_juega_con_criterio_evitar_mov_ganador
  (declare (salience -9997))
  ?f <- (Turno X)
  (Todas_fichas_en_tablero X)
  (Puede_ganar ?jug_i ?jug_j ?destino_i ?destino_j O)
  (Conectado ?destino_i ?destino_j ?forma ?origen_i ?origen_j)
  (Posicion ?origen_i ?origen_j X)
    =>
  (assert (Juega X ?origen_i ?origen_j ?destino_i ?destino_j))
  (printout t "Juego mover la ficha de "  ?origen_i ?origen_j " a " ?destino_i ?destino_j crlf)
...
(defrule clisp_juega_con_criterio
  ...
  (Posicion ?destino_i ?destino_j " ")
  (Conectado ?origen_i ?origen_j ? ?destino_i ?destino_j)
  (2_en_linea ?forma ?i1 ?j1 ?i2 ?j2 O)
  (Conectado ?origen_i ?origen_j ?forma ?i1 ?j1)
  (Conectado ?origen_i ?origen_j ? ?i3 ?j3)
  (Posicion ?i3 ?j3 X)
    =>
  (assert (Juega X ?origen_i ?origen_j ?destino_i ?destino_j))
  (printout t "Juego mover la ficha de "  ?origen_i ?origen_j " a " ?destino_i ?destino_j crlf)
